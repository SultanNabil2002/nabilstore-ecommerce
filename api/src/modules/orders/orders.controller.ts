...
export class OrdersController {
    ...
    @RelaxedThrottle()
    @ApiOperation({
        summary: "[ADMIN] Get all order (paginated)"
    })
    @ApiQuery({
    ...
    @ApiQuery({
        name: 'search', required: false, type: String, description: 'Search by Order ID or Order Number'
    })
    @ApiResponse({
    ...
    @ApiForbiddenResponse({
        description: 'Admin access requried'
    })
    ...
    @RelaxedThrottle()
    @ApiOperation({
        summary: 'Get an order byID for current user'
    })
    @ApiParam({
        name: 'id',
    ...
    async update(
        @Param('id') id: string,
        @Body() dto: UpdateOrderDto, @GetUser('id') userId: string) {
